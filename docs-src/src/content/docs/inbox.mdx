...
<ProofSequence
  title="Prepare autonomously, pause narrowly"
  description="The Brightside demo keeps preparation and human authority visibly separate."
  items={[
    { title: 'The boundary is explicit', src: '/docs/proof/ordinary-request.webp', alt: 'A business request that says not to send or publish anything', caption: 'The session may prepare and improve the work, but publication remains outside its authority.' },
    { title: 'The remaining decision is specific', src: '/docs/proof/browser-result-focused.webp', alt: 'A reviewed status page with a single launch-window decision', caption: 'The result is already inspectable; only the launch window needs human judgment.' },
  ]}
/>
