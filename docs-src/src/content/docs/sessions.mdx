...
<ProofSequence
  title="See a session while it works"
  description="Each image demonstrates a different session state, not a repeated overview."
  items={[
    { title: 'Inspect active work', src: '/docs/proof/working-expanded.webp', alt: 'An expanded Working fold showing chronological reads, commands, searches, and browser actions', caption: 'Open Working to follow the agent’s reasoning and actions in chronological order.' },
    { title: 'Queue the next instruction', src: '/docs/proof/steering-queue.webp', alt: 'An active Axis session with a follow-up waiting in the steering queue above the composer', caption: 'A follow-up waits visibly while the current turn continues, ready to reorder, edit, delete, or steer through.' },
    { title: 'Review the evidence', src: '/docs/proof/validation-evidence.webp', alt: 'Validation metrics and exact diff evidence retained in the same session', caption: 'When work settles, its measurable proof stays attached to the thread.' },
  ]}
/>
